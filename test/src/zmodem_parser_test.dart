...
void main() async {
  ...
  group('ZModemParser', () {
    // The parser does NOT parse data subpackets on its own: after a
    // ZBIN header whose type carries a payload, the DRIVER (ZModemCore)
    // must arm exactly one subpacket parse via expectDataSubpacket().
    // These tests mirror that contract; without arming, payload bytes
    // are consumed as plain text and only headers are yielded.
    test('works', () async {
      final parser = ZModemParser();
      parser.addData(session1Data);

      for (final expectedPacket in session1Packets) {
        if (expectedPacket is ZModemDataPacket) {
